...
# Initialize DB on startup (resets every time app starts)
init_db()
...
def complete_todo(todo_id: int):
    conn = get_db_connection()
    cur = conn.execute("UPDATE todos SET completed = 1 WHERE id = ?", (todo_id,))
    if cur.rowcount == 0:
        conn.close()
...
def update_todo(todo_id: int, title: str = Form(...), description: str = Form("")):
    conn = get_db_connection()
    cur = conn.execute(
